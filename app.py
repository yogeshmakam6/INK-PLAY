...
def init_db():
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY, email TEXT, password TEXT, created TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS scripts (username TEXT, script_name TEXT, content TEXT, updated TEXT, PRIMARY KEY (username, script_name))''')
    c.execute('''CREATE TABLE IF NOT EXISTS history (id SERIAL PRIMARY KEY, username TEXT, script_name TEXT, content TEXT, saved TEXT)''')
    conn.commit()
    conn.close()
    return True
...
def global_css(dark):
    ...
    *, *::before, *::after {{ box-sizing: border-box !important; }}
    html, body {{ margin: 0 !important; padding: 0 !important; background: {BG} !important; color: {FG} !important; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif !important; font-weight: 700 !important; -webkit-font-smoothing: antialiased !important; }}
    .stApp, [data-testid="stAppViewContainer"], [data-testid="stAppViewBlockContainer"], section.main {{ background: {BG} !important; }}
    .block-container {{ background: {BG} !important; padding: 2rem 2.5rem 2rem 2.5rem !important; max-width: 100% !important; }}
    [data-testid="stSidebar"] {{ display: none !important; }}
    p, span, div, label, small, caption, li, td, th, h1, h2, h3, h4, h5, h6 {{ font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif !important; font-weight: 700 !important; color: {FG} !important; }}
    .stTextInput label {{ display: none !important; }}
    .stTextInput > div > div {{ background: transparent !important; border: none !important; }}
    .stTextInput div[data-baseweb="input"] {{ background-color: {INP} !important; border: 1px solid {BRD} !important; border-radius: 24px !important; overflow: hidden !important; }}
    .stTextInput div[data-baseweb="input"]:focus-within {{ border-color: {FG} !important; }}
    .stTextInput div[data-baseweb="input"] * {{ background-color: transparent !important; border: none !important; }}
    .stTextInput div[data-baseweb="input"] input {{ color: {FG} !important; font-size: 13px !important; padding: 12px 18px !important; caret-color: {FG} !important; }}
    textarea {{ background: {INP} !important; color: {FG} !important; border: 1px solid {BRD} !important; border-radius: 4px !important; font-family: 'Courier New', Courier, monospace !important; font-size: 14px !important; line-height: 1.7 !important; padding: 16px !important; outline: none !important; resize: vertical !important; }}
    textarea:focus {{ border-color: {FG} !important; }}
    .stButton > button, [data-testid="stFormSubmitButton"] > button {{ white-space: nowrap !important; background: transparent !important; color: {FG} !important; border: 1px solid {BRD} !important; border-radius: 24px !important; font-size: 10px !important; font-weight: 600 !important; letter-spacing: .08em !important; padding: 8px 12px !important; cursor: pointer !important; transition: all .2s ease !important; width: 100% !important; min-width: max-content !important; text-transform: uppercase !important; }}
    .stButton > button:hover, [data-testid="stFormSubmitButton"] > button:hover {{ background: {FG} !important; color: {BG} !important; border-color: {FG} !important; }}
    ...
    #MainMenu, header, footer, [data-testid="stToolbar"] {{ display: none !important; visibility: hidden !important; }}
...
def page_auth():
    ...
    st.markdown("""<style>.block-container { display: flex !important; align-items: center !important; justify-content: center !important; min-height: 100vh !important; padding: 0 !important; max-width: 400px !important; margin: 0 auto !important; } section.main > div { display: flex !important; align-items: center !important; min-height: 100vh !important; }</style>""", unsafe_allow_html=True)
    ...
    if st.session_state.reg_step == 2:
        ...
        with st.form("verify_form"):
            user_otp = st.text_input("ENTER OTP", placeholder="6-digit OTP")
            submit_verify = st.form_submit_button("VERIFY", use_container_width=True)
    ...
    with st.form("auth_form"):
        ...
        submit_auth = st.form_submit_button(btn_label, use_container_width=True)
...
def page_dashboard():
    ...
    with st.form("new_script_form"):
        ...
        with col_inp:
            nname = st.text_input("N", placeholder="Enter your new script title here...", label_visibility="collapsed")
        with col_btn:
            submit_script = st.form_submit_button("➕ CREATE", use_container_width=True)
            if submit_script:
                n = nname.strip()
                if n:
...
def page_app():
    ...
    with col_back:
        if st.button("← DASHBOARD", use_container_width=True):
            with st.spinner("Saving and returning..."):
                st.session_state.page = "dashboard"
                st.rerun()
    with col_spc1: st.empty()
    with col_brand:
    ...
    with col_t:
        ...
        st.markdown(f"<div style='margin-bottom: 4px; padding-top:2px;'><span style='font-weight:700; font-size:1.1rem; letter-spacing:.14em; color:{FG};'>{sname.upper()}</span></div>", unsafe_allow_html=True)
    with col_stats:
    ...
    with tab_w:
        ...
        new_text = st.text_area("W", value=content, height=620, label_visibility="collapsed", key=f"ta_{sname}")
        if new_text != content:
    ...
    with tab_scenes:
        ...
        if scenes:
            st.markdown(f"<div style='font-size:.6rem;letter-spacing:.14em;color:{SUB};margin-bottom:14px;'>{len(scenes)} SCENE{'S' if len(scenes)!=1 else ''}</div>", unsafe_allow_html=True)
            for ln, heading in scenes:
                st.markdown(f"<div style='font-size:.8rem;font-weight:700;letter-spacing:.08em;color:{FG};padding:4px 0;border-bottom:1px solid {BRD};'>{heading}</div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<p style='font-size:.75rem;color:{SUB};text-align:center;margin-top:60px;letter-spacing:.14em;'>NO SCENES YET.</p>", unsafe_allow_html=True)
    with tab_hist:
        ...
        if revs:
            for idx, rev in reversed(list(enumerate(revs))):
                ...
                with col_info:
                    st.markdown(f"<div style='padding:8px 0;border-bottom:1px solid {BRD};'><div style='font-size:.65rem;letter-spacing:.1em;color:{SUB};margin-bottom:3px;'>{ts_rev}</div></div>", unsafe_allow_html=True)
                with col_btn:
    ...
    with tab_p:
        if content.strip():
            st.markdown(f"<div style='background:{INP}; border:1px solid {BRD}; padding:32px 44px; max-height:580px; overflow-y:auto; line-height:1.75;'>{content.replace(chr(10), '<br>')}</div>", unsafe_allow_html=True)
...
def page_settings():
    ...
    with col_back:
        if st.button("← BACK", use_container_width=True, key="settings_back"):
            with st.spinner("Returning to dashboard..."):
                st.session_state.page = "dashboard"
                st.rerun()
    ...
    if st.session_state.pw_step == 1:
        ...
        with st.form("req_otp_form"):
            submit_req = st.form_submit_button("SEND OTP TO MY EMAIL", use_container_width=True)
            if submit_req:
        ...
        with st.form("update_pw_form"):
            pw_otp  = st.text_input("OTP", placeholder="6-digit OTP")
            new_pw  = st.text_input("NEW PASSWORD", type="password", placeholder="6+ characters")
            new_pw2 = st.text_input("CONFIRM NEW PASSWORD", type="password")
            
            submit_pw = st.form_submit_button("UPDATE PASSWORD", use_container_width=True)
            if submit_pw:
                entered_otp = str(pw_otp).strip()
